...
kakovost urnika:
\[k_1 \cdot u_1 + k_2 \cdot u_2 + \dots + k_(n-1) + k_n \cdot u_n\]
...
\subsection{Avtomatizirana izdelava urnika} % razdelek
...
\begin{enumerate}
   ...
   \item Za vse proste termine \(\text{predavalnica}_{i}\) izračunamo ceno vstavljanja
...
spremembo npr. zamenjava \(text{srečanja}_{i}\) in \(text{srečanja}_{j}\) sedaj shranimo v
